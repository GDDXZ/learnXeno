...
void myTask_proc(void * arg)
{
     ...
     * protocol namespace.
     */
    s = rt_dev_socket(AF_RTIPC, SOCK_DGRAM, IPCPROTO_XDDP);
    if (s < 0) {
        perror("socket");
        exit(EXIT_FAILURE);
    }
     ...
     */
    streamsz = 1024; /* bytes */
    ret = rt_dev_setsockopt(s, SOL_XDDP, XDDP_BUFSZ,
    ...
    ret = rt_dev_bind(s, (struct sockaddr *)&saddr, sizeof(saddr));
    if (ret)
        fail("bind");
    ...
    for (;;) {
         ...
         */
        for (b = 0; b < len; b++) {
            ret = rt_dev_sendto(s, msg[n] + b, 1, MSG_MORE, NULL, 0);
            if (ret != 1)
                fail("sendto");
        }
        ...
        ret = rt_dev_recvfrom(s, buf, sizeof(buf), 0, NULL, 0);
        if (ret <= 0)
            fail("recvfrom");
...
int main(int argc, char **argv)
{
    ...
    errno = rt_task_create(&myTask, "myTask", 0, 90, T_FPU);
    if (errno)
        fail("rt_task create");

    errno = rt_task_start(&myTask, &myTask_proc, NULL);
    if (errno)
        fail("rt_task start");
